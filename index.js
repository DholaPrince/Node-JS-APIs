...
//get locations-->http://localhost:5500/locations

app.get('/locations', (req, res) => {
    db.collection("newLocation").find().toArray((err, result) => {
        if (err) throw err;
        res.send(result)
    })
...
//get mealType-->http://localhost:5500/quickSearch

app.get('/quickSearch', (req, res) => {
    db.collection("mymealType").find().toArray((err, result) => {
        if (err) throw err;
        res.send(result)
    })
...
//get restaurant data-->http://localhost:5500/restaurants
...
app.get('/restaurants', (req, res) => {
    ...
    db.collection("NewRestaurant").find(query).toArray((err, result) => {
        if (err) throw err;
        res.send(result)
    })
...
app.get('/filter/:mealId', (req, res) => {
    ...
    db.collection("NewRestaurant").find(query).sort(sort).toArray((err, result) => {
        if (err) throw err;
        res.send(result)
    })
...
app.get('/details/:id', (req, res) => {
    ...
    db.collection("NewRestaurant").find({ restaurant_id: id }).toArray((err, result) => {
        if (err) throw err;
        res.send(result)
    })
